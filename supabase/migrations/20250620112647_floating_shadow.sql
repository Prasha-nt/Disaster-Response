/*
  ...
  1. New Tables
    ...
    - `resources` - Emergency resources (shelters, medical, supplies) with locations
    - `cache` - API response caching with TTL support

  2. Security
    - Enable RLS on all tables
    - Add policies for authenticated and anonymous users
    - Demo-friendly policies (restrict in production)

  3. Performance
    - Geospatial indexes using GIST for location queries
    - GIN indexes for array fields (tags)
    - Standard B-tree indexes for foreign keys and timestamps

  4. Functions
    - `get_nearby_resources` - Geospatial query for resources within radius
    - `clean_expired_cache` - Maintenance function for cache cleanup
...
CREATE OR REPLACE FUNCTION get_nearby_resources(
  ...
  filter_disaster_id uuid DEFAULT NULL
)
RETURNS TABLE (
  id uuid,
  disaster_id uuid,
...
BEGIN
  ...
  WHERE 
    ...
    AND (filter_disaster_id IS NULL OR r.disaster_id = filter_disaster_id)
